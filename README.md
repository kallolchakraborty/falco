...
- **Routing** (`Router`, `Route`, `RouteMatch`): O(n) linear scan; path templates use `{name}` captured as `[^/]` — no backtracking surprises, no regex injection. `matchTemplate()` builds a single anchored PCRE per route.
